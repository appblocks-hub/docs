...
Appblocks enforces strict standard regulations as there are increased chances of runtime errors. It ensures that all the element Blocks which are supposed to be connected are composable by design.
...
Appblocks gateway in the proxy layer will intelligently route the traffic between the stitched app and the isolated Fn Block instance.
...
This configuration can be pulled and run on any machine using BB CLI. We call this hypothetical entity an `Package Block`. 
...
This `configuration file is at the root of any Package Block`. 

Appblock.config.json will have `meta-information about all the Blocks in that package block` and other `necessary metadata about its runtime`. 
